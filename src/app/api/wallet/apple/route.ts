...
import {
  ...
  getPassPreviewData,
} from '@/lib/services/wallet/apple-wallet'
...
export async function GET() {
  try {
    ...
    if (!isAppleWalletConfigured()) {
      // Return preview data instead
      return NextResponse.json({
        success: false,
        configured: false,
        preview: getPassPreviewData(passData),
        message: 'Apple Wallet certificates not configured. Showing preview only.',
      })
    }
    ...
    if (!result.success || !result.buffer) {
      return NextResponse.json(
        { success: false, error: result.error || 'Failed to generate pass' },
        { status: 500 }
      )
    }
    ...
    return new NextResponse(new Uint8Array(result.buffer), {
      headers: {
        ...
        'Content-Disposition': `attachment; filename="corbe-pass.pkpass"`,
      },
    })
  } catch (error) {
